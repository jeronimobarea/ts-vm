...
const OpCodes: {
  ...
  0x54: new Instruction(0x54, 'SLOAD', async (ctx: ExecutionContext) => {
    ...
    const value = await ctx.storage.get(bigIntToBytes(key))
    ctx.stack.push(value ? bytesToBigInt(value) : BigInt(0))
  }),
  0x55: new Instruction(0x55, 'SSTORE', async (ctx: ExecutionContext) => {
    const [key, value] = [ctx.stack.pop(), ctx.stack.pop()]
    await ctx.storage.put(
      setLengthLeft(bigIntToBytes(key), 32),
      bigIntToBytes(value),
    )
  }),
  ...
  0xf3: new Instruction(0xf3, 'RETURN', (ctx: ExecutionContext) => {
    ...
    const hexOutput = arrayify(output.toString(16)).slice(0, Number(size))
    ctx.output = BigInt(hexlify(hexOutput))
  }),
}
